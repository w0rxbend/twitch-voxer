...
sequenceDiagram
    ...
    app->>sto: await load() on all three  ← the root loads what it constructed
...
| Stores loaded by the composition root, not by `MessageHandler` | Reading a pickledb file is I/O and has to be awaited, and `async def __init__` is not valid Python, so a store cannot fill itself in its own constructor. `MessageHandler` used to carry a `preload_resources()` method for it, which put a class that merely *reads* the three stores in charge of their startup. `app.py` is what constructs them, so it is also what loads them — each `await store.load()` sits directly under the line that created it, before the handler that consumes them is built. |
